...
<form id="ntcListForm">
	...
	<table border="1"  style="text-align: center;">
		...
		<tr>
			<td colspan="5"> 총 ${listCount}건 / ${totalPageCount}페이지
				...
				<div style="float: right;">
					<select name="listsize" id="listsize" onchange="javascript:ntcSearch();">
		...
		<!-- 등록 된 게시물이 있을 때 -->
		<c:if test="${totalPageCount > 0}">
			<c:forEach items="${ntcList}" var="ntcList">
			<tr>
				<td>${ntcList.NTC_SN}</td>
				<td>
					<a href="javascript:ntcDetail('${ntcList.NTC_SN}');">${ntcList.NTC_TITLE}</a>
				</td>
				<td>${ntcList.USER_ID}</td>
				<td>${ntcList.REG_DT}</td>
				<td>${ntcList.NTC_HIT}</td>
			</tr>	 
			</c:forEach>
			...
			<tr>
				<td colspan="5">
					<c:if test="${startPage > 10}">
						<a href="ntcSetPageNum('${startPage-1}');">◀</a>
					</c:if>
					<c:forEach var="pageno" begin="${startPage}" end="${endPage}">
						...
						<c:if test="${pageNum ne pageno}">
							<a href="javascript:ntcSetPageNum('${pageno}');">[${pageno}]</a>
						</c:if>
					</c:forEach>
					<c:if test="${endPage < totalPage}">
						<a href="javascript:ntcSetPageNum('${endPage+1}');">▶</a>
					</c:if>
				</td>
			</tr>
		</c:if>
		...
		<tr>
			<td colspan="5" align="right">
				...
				<input type="button" value="조회" onclick="javascript:ntcSearch();">
			</td>
		</tr>
		...
		<c:if test="${not empty sessionScope.user_auth}">
			<tr>
				<td colspan="5" align="right">
					<a href="javascript:ntcWrite();">글쓰기</a>
				</td>
			</tr>
		</c:if>
